		...
		function bindDeptDisbled(disabled, deptId)
		{
			...
			$(label).click(function(){
				ajaxData(url, {id:deptId}, function(){window.location.href="/employee/list.htm"});
			});
			return label;
		}
        ...
        function showDataList(data)
        {
        	...
        	$(list).each(function(k,v){
        		...
        		$(tr).find("td:eq(4)").text(getDept(v.departmentId).name);
        		$(tr).find("td:eq(5)").text(v.manager?"是":"否");
        		$(tr).find("td:eq(6)").text(v.superAdmin?"是":"否");
				var select = $(tr).find("td:eq(7) > select");
        		select.val(v.status);
				//下拉提交事件
        		...
        		$(tr).find("td:eq(8) > button").click(function () {
        			...
        			$("#departmentId").val(v.departmentId);
        			$("#manager").prop("checked", v.manager ? "checked" : "");
